...
Make the generic DGX Spark platform understandable to a new operator without
requiring them to read historical plans, confuse the temporary SSH transport
with the production architecture, or infer which two-Spark instructions remain
...
The primary reader operates a small fleet of one or more DGX Sparks and a
separate Docker Compose-capable service host. They may be starting from an
empty installation or migrating the repository's original two-Spark setup.
...
3. onboard the first Spark and repeat the same process for every addition;
...
will describe the service-host and Spark roles, state that fleet size is not
...
- Legacy and reference: historical two-Spark inventory, fabric, and bootstrap
...
3. onboard exactly one Spark with `spark-install`;
...
6. repeat onboarding independently for every additional Spark;
...
8. preview, validate, submit, and reconcile an eligible commit through the
   control API and agent job path;
9. operate and update through the web UX or `sparkctl admin`; and
10. complete physical and protected-code-host gates before the first release.

Legacy `inventory/cluster.toml`, `spark1`/`spark2`, fixed addresses, and the
...
`2026-08-03-outbound-spark-agent-design.md` and must land before documents mark
