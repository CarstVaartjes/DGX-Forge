...
**Goal:** Deliver the installable Vonk Forge control plane and Rust Spark agent together with the public `vonk-forge-web` recipe catalog, publishing API, and website.
...
- The Spark agent, stable supervisor, and privileged helper are Rust binaries delivered in a signed ARM64 `.deb`.
- Spark agents initiate outbound mTLS long polling and expose no routine inbound management endpoint.
...
- LiteLLM reaches only the validated Spark entrypoint on the restricted management LAN; worker ranks use the Spark fabric.
- The global service never connects to a local database, controller, agent, Spark, tailnet, or model endpoint.
...
| 3 | `2026-08-07-vonk-sparkrun-import-and-runtime.md` | `vonk-forge` | local catalog | Exhaustive SparkRun import report and typed vLLM/SGLang/llama.cpp compiler |
...
- **Agent gate:** Python-oracle and Rust-agent protocol/failure fixtures match; physical Spark install and rollback pass.
...
4. SparkRun fixture coverage by registry/runtime/import disposition;
...
8. exact route-generation evidence showing Caddy, LiteLLM, controller, and Spark responsibilities; and
