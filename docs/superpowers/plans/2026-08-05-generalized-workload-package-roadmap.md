...
| Workload control plane | W11–W16 | Read/projection/UI and worker-owned removal/GC/rollout dispatch complete; publication and validation remain trust-gated | Alembic `0014_package_action_plans`, Git/SQL projections, discovery/resolution, typed API/CLI/web, TUF-authorized rollout jobs, digest-bound removal/GC, and durable promotion preview/apply fencing; publication still requires the isolated signer and validation still requires its runner |
