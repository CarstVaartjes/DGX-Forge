...
**Architecture:** Tasks 26–31 remain the Vonk Forge platform-update lane and now include authenticated version-skew telemetry plus the NAS-newer update prompt. A separately numbered W1–W20 workload lane owns generic workload contracts, GPU node package execution, NAS administration, migration, and acceptance. W21 is a focused UX/resource presentation lane that consumes the W15/W16 API and does not create a second control plane. Tasks 32–37 remain final platform hardening but consume W20/W21 evidence before the first real release.
...
Task 25 complete -> landed GitHub container-release baseline (`e9f7695`)
  -> Tasks 26-31: Vonk Forge platform update and NAS/GPU node version-skew UX
      -> W1-W4: contracts, Git authority, workload TUF, promotion
          -> W5-W10: GPU node acquisition, store, environments, adapters, generations
              -> W11-W16: discovery, database, validation, reconciliation, API/CLI/web
                  -> W17-W20: Mia/DS4 migration, unknown-package E2E, hardening, docs
                      -> W21: GPU node inventory, capacity UX, safe removal, visual polish
...
[`2026-08-05-node-workload-package-engine.md`](2026-08-05-node-workload-package-engine.md).
...
| GPU node package engine | W5–W8, W10 | Complete | Agent/protocol suites, resumable acquisition, immutable environments, generic adapter ABI, generations, rollback, and GC |
...
| Workload control plane | W11–W16 | Read/projection/UI, worker-owned validation, removal/GC, and rollout dispatch complete; publication remains trust-gated | Alembic `0014_package_action_plans`, Git/SQL projections, discovery/resolution, typed API/CLI/web, TUF-authorized rollout jobs, digest-bound removal/GC, generic deployment-bound validation through the authenticated agent queue, and durable promotion preview/apply fencing; publication still requires the isolated signer |
...
| Operator experience | W21 | Implemented locally; acceptance evidence landed | Per-GPU node inventory, resource-envelope/co-residency projections, digest-bound removal/GC dispatch, rollback identity, and polished responsive web UX; focused API/worker tests and web build are green |
...
checkpoint, or adapter release is therefore independent of the Vonk Forge
...
current GPU node helper executes native workloads and Python virtual environments
...
| Shared immutable workload lock bytes | None | W1 | `vonk_agent_protocol` is the sole shared wire owner. |
...
| Current `node-runtime-v1` adapter path | Accepted legacy migration source | W9/W17 | Keep until generic equivalence passes; never expand its hard-coded catalog. |
...
| GPU node inventory, capacity envelope, safe removal, and polished web presentation | W15/W16 typed projections | W21 | W21 consumes the existing API/CLI/job contracts; it must not add a second reconciler, trust root, or model catalog. |
...
| GPU node content store, reservations, resumable providers, deduplication | W6, W7 |
| Signed resource envelopes, KV/cache sizing, storage headroom, and co-residency admission | W1–W2, W6–W7, W13–W16 |
| Single/replicated/gang topology, rank/world-size/fabric fencing, and atomic multi-GPU node lifecycle | W1–W2, W5, W14–W16 |
...
| Capacity envelope, co-residency/distributed placement, per-node inventory, and safe removal | W6, W10, W14–W16, W21 |
| Failure taxonomy, recovery, cancellation, scale, and security | W5, W10, W12, W19 |
| Platform/workload release boundary and NAS-newer GPU node update prompt | Tasks 26–31, W20 |
...
- a per-node inventory of downloading, staged, available, active, retained,
...
privileged runc capability and evidence are installed on the GPU node.
...
replicas and barriered multi-GPU node gangs with certificate-bound roles/ranks,
world size, and fabric requirements.  Per-GPU node inventory and explicit
...
Require staged/downloaded versus active inventory, per-node storage and
...
Run whole-repository threat, backup/restore, service-host loss, GPU node recovery,
