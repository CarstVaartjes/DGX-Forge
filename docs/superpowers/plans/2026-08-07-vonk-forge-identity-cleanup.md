...
**Goal:** Remove legacy Spark/DGX terminology from Vonk-owned identifiers and
migrate the unreleased repository to one consistent Vonk Forge identity.
...
- Vonk-owned names use Vonk Forge, `vonk-forge`, `VONK_*`, `node`, GPU host,
  cluster, and workload.
- Vonk-owned code, configuration, paths, commands, API schemas, fixtures, and
  docs contain no legacy `spark` or `dgx` tokens.
- External evidence is not renamed. It may retain legacy tokens only where the
  text is clearly a vendor/upstream identifier, command, URL, model name, or
  raw hardware record. This includes NVIDIA `dgx-spark-playbooks`, its
  `discover-sparks` command, NVIDIA documentation URLs, and the explicit
  evidence roots `manifests/`, `inventory/raw/`, and
  `tests/fixtures/external/`. Such text records provenance; it is never a
  Vonk-owned service, path, command, or configuration name.
...
| Legacy Vonk-owned identifier | Canonical identifier |
| --- | --- |
| `spark_profiles` | `cluster_profiles` |
| `sparkctl` | `vonkctl` |
| `dgx_control` | `vonk_control` |
| `dgx_agent` | `vonk_agent` |
| `dgx_agent_protocol` | `vonk_agent_protocol` |
| `dgx-forge-*` services, packages, and images | `vonk-forge-*` |
| `DGX_*` Compose/application settings | `VONK_*` |
| `dgx-forge-control` | `vonk-forge-control` |
| `SparkRun` and spark profiles | `WorkloadRun` and workload profiles |

The mapping applies only to Vonk-owned namespace material. NVIDIA DGX Spark
names in cited upstream documentation, pinned repository names, and recorded
operator evidence remain exact so that operators can locate and execute the
vendor procedure.
...
- [ ] Step 3: Render Compose with the checked-in test environment.
    ...
    Expected: no `DGX_` or legacy path/image/hostname requirement; required
    canonical `VONK_*` settings remain present.
