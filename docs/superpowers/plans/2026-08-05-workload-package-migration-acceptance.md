...
> **Implementation status (2026-08-06): complete locally for the installed
> backend capability.** W17–W20 are implemented on `main`, with generic Mia/DS4
> projections, unknown-family E2E, failure/scale/security acceptance, operator
> documentation, and release evidence. The migrated OCI declarations are
> rejected fail-closed until the reviewed OCI runtime capability is installed;
> physical/protected-host release gates remain intentionally external. See the
> [roadmap status ledger](2026-08-05-generalized-workload-package-roadmap.md#implementation-status-2026-08-06).
