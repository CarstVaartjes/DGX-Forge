...
**Goal:** Generalize inventory, health, placement, backend targeting, profiles, and deployment from fixed `node1`/`node2` assumptions to configured node IDs without changing accepted two-GPU node runtime behavior.
...
- Preserve current two-workload profile behavior and evidence hashes through compatibility readers.
...
**Files:**
- Modify only after upstream lands: `src/cluster_profiles/ssh_transport.py`
- Modify: `src/cluster_profiles/install/remote.py`
- Test: `tests/cluster_profiles/install/test_remote.py`
- Test: `tests/cluster_profiles/test_ssh_transport.py`
...
Run: `git log --all -- src/cluster_profiles/ssh_transport.py && git status --short`
...
Run: `uv run pytest tests/cluster_profiles/test_ssh_transport.py tests/cluster_profiles/test_backend.py tests/scripts/test_deploy_runtime_release.py tests/scripts/test_validate_fabric.py -v`
...
Run: `uv run pytest tests/cluster_profiles/install/test_remote.py tests/cluster_profiles/test_ssh_transport.py tests/cluster_profiles/test_backend.py tests/scripts/test_deploy_runtime_release.py tests/scripts/test_validate_fabric.py -v`
Expected: PASS.
...
**Files:**
- Modify: `src/cluster_profiles/backend.py`
- Test: `tests/cluster_profiles/test_backend.py`
...
def test_backend_rejects_unknown_node_without_default_alias(fleet_16):
    with pytest.raises(ValueError, match="unknown node"):
        SshBackend.from_fleet(fleet_16).run("node1", ("true",), 10)
```

- [ ] **Step 2: Run and verify current two-alias default fails**

Run: `uv run pytest tests/cluster_profiles/test_backend.py -v`
...
Run: `uv run pytest tests/cluster_profiles/test_backend.py tests/cluster_profiles/test_ssh_transport.py -v`
Expected: PASS.

- [ ] **Step 5: Commit backend generalization**

```bash
git add src/cluster_profiles/backend.py tests/cluster_profiles/test_backend.py
git commit -m "feat: target fleet-defined GPU node endpoints"
```

### Task 3: Generalize health collection and schemas

**Files:**
- Modify: `src/cluster_profiles/health.py`
- Modify: `schemas/node-health.schema.json`
- Modify: `src/cluster_profiles/schemas/node-health.schema.json`
- Test: `tests/cluster_profiles/test_health.py`
...
Run: `uv run pytest tests/cluster_profiles/test_health.py -v`
Expected: FAIL on the current `exactly node1 and node2` requirement.
...
Run: `cmp schemas/node-health.schema.json src/cluster_profiles/schemas/node-health.schema.json && uv run pytest tests/cluster_profiles/test_health.py -v`
Expected: PASS.

- [ ] **Step 5: Commit health generalization**

```bash
git add src/cluster_profiles/health.py schemas/node-health.schema.json src/cluster_profiles/schemas/node-health.schema.json tests/cluster_profiles/test_health.py
git commit -m "feat: collect health across generic GPU node fleets"
```

### Task 4: Add requirement-based deterministic placement

**Files:**
- Create: `src/cluster_profiles/placement.py`
- Create: `schemas/placement-requirements.schema.json`
- Create: `src/cluster_profiles/schemas/placement-requirements.schema.json`
- Test: `tests/cluster_profiles/test_placement.py`
...
Run: `uv run pytest tests/cluster_profiles/test_placement.py -v`
Expected: FAIL.
...
Run: `cmp schemas/placement-requirements.schema.json src/cluster_profiles/schemas/placement-requirements.schema.json && uv run pytest tests/cluster_profiles/test_placement.py -v`
Expected: PASS.

- [ ] **Step 5: Commit planner**

```bash
git add src/cluster_profiles/placement.py schemas/placement-requirements.schema.json src/cluster_profiles/schemas/placement-requirements.schema.json tests/cluster_profiles/test_placement.py pyproject.toml
...
- Modify: `src/cluster_profiles/admission.py`
- Create: `src/cluster_profiles/profile_compat.py`
- Create: `schemas/cluster-profile-v2.schema.json`
- Create: `schemas/workload-v2.schema.json`
- Mirror schemas under: `src/cluster_profiles/schemas/`
- Test: `tests/cluster_profiles/test_profile_compat.py`
- Modify: `pyproject.toml`
...
def test_v2_profile_has_no_legacy_named_properties(v2_schema):
...
Run: `uv run pytest tests/cluster_profiles/test_profile_compat.py -v`
Expected: FAIL.
...
Run: `uv run pytest tests/cluster_profiles/test_contracts.py tests/cluster_profiles/test_admission.py tests/cluster_profiles/test_catalog.py tests/cluster_profiles/test_profile_compat.py -v`
Expected: PASS.

- [ ] **Step 5: Commit contract migration**

```bash
git add src/cluster_profiles schemas src/cluster_profiles/schemas tests/cluster_profiles/test_profile_compat.py pyproject.toml
...
- Test: `tests/cluster_profiles/test_cli.py`
- Test: `tests/cluster_profiles/test_switcher.py`
...
Run: `uv run pytest tests/cluster_profiles/test_cli.py tests/cluster_profiles/test_switcher.py -v`
Expected: FAIL where code iterates `node1`, `node2`.
...
Run: `uv run pytest tests/cluster_profiles -v && uv run pytest -q`
Expected: all tests PASS.

- [ ] **Step 5: Commit N-node controller**

```bash
git add src/cluster_profiles/cli.py src/cluster_profiles/switcher.py tests/cluster_profiles/test_cli.py tests/cluster_profiles/test_switcher.py
...
def test_release_targets_resolved_generic_nodes(generic_release_root, run_release):
    ...
    assert all(alias not in result.stdout for alias in ("vonk-node-1", "vonk-node-2"))
```
