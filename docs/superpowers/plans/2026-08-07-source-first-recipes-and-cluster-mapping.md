...
**Goal:** Replace the unreleased image-first recipe draft with one source-first v1 contract, build each workload once on a Spark, distribute that exact OCI result, and bind portable deployment profiles to first-class local cluster mappings.

**Architecture:** Recipe JSON and an immutable source bundle are the portable input. A local build record binds those inputs to one OCI digest; a cluster mapping binds one recipe profile to exact Spark identities, roles, and ranks; installations and fenced runs consume that mapping without changing recipe meaning. The global service stores recipe metadata plus source bundles and never requires a community workload registry.
...
- SparkRun is translated, never invoked as Vonk's scheduler.
...
- Modify (`vonk-forge`): `control/src/dgx_control/recipe_contract.py`
- Create (`vonk-forge`): `control/src/dgx_control/source_bundles.py`
- Modify (`vonk-forge`): `control/src/dgx_control/models.py`
...
### Task 2: Make catalog metadata and SparkRun imports source-first

**Files:**
- Modify: `control/src/dgx_control/catalog_service.py`
- Modify: `control/src/dgx_control/catalog_api.py`
- Modify: `control/src/dgx_control/artifact_sizes.py`
- Modify: `control/src/dgx_control/sparkrun_source.py`
- Modify: `control/src/dgx_control/import_report.py`
- Modify: `control/src/dgx_control/sparkrun_importer.py`
- Modify: `control/src/dgx_control/import_resolution.py`
- Modify: `control/src/dgx_control/sparkrun_workflow.py`
- Modify: `control/src/dgx_control/runtime_compilers/common.py`
...
- Modify: `control/tests/test_sparkrun_source.py`
- Modify: `control/tests/test_sparkrun_importer.py`
- Modify: `control/tests/test_import_resolution.py`
- Modify: `control/tests/test_sparkrun_api.py`
- Add broad profiles under: `control/tests/fixtures/sparkrun/`
...
- `SparkRunImportResult` adds `bundle: GeneratedSourceBundle`.
...
def test_container_and_mods_become_source_bundle(parsed_profile) -> None:
    result = import_sparkrun(parsed_profile)
...
Run: `uv run --project control pytest control/tests/test_catalog_service.py control/tests/test_catalog_api.py control/tests/test_sparkrun_importer.py control/tests/test_import_resolution.py -q`.
...
- [ ] **Step 4: Generate a wrapper bundle during SparkRun import**
...
- [ ] **Step 5: Cover the live SparkRun surface and commit**

Add fixtures for vLLM distributed/Ray, SGLang, llama.cpp, TensorRT-LLM, arbitrary defaults, environment references, runtime configuration, mods, tuning, and benchmarks. Run all SparkRun/compiler/import/catalog tests.

Commit: `git commit -m "feat: import SparkRun as source recipes"`.
...
- Modify: `control/src/dgx_control/recipe_operation_worker.py`
...
- Replace: `control/src/dgx_control/recipe_deployments.py` with `control/src/dgx_control/cluster_mappings.py`
- Create: `control/tests/test_cluster_mappings.py`
- Modify: `control/src/dgx_control/install_admission.py`
- Modify: `control/src/dgx_control/run_admission.py`
- Modify: `control/src/dgx_control/topology.py`
- Modify: `control/src/dgx_control/recipe_operations.py`
...
Place artifacts and disk per role; reserve startup, steady, growth, and system memory by declared memory kind; count Spark unified memory once. Validate `none`, `connected`, `full_mesh`, and `switch` fabric requirements for arbitrary positive counts. Test a three-node group fence and compensation.
...
**Files (`vonk-forge`):**
- Modify: `control/src/dgx_control/global_catalog.py`
- Modify: `control/src/dgx_control/catalog_service.py`
...
- `/catalog/new` offers SparkRun import, standard clone, and fully custom.
...
```tsx
it.each(["Import SparkRun", "Start from a standard", "Fully custom"])(
...
All origins edit overview, source, artifacts, parameters, runtime, profiles, resources, validation, and provenance. Mapping preview shows exact Sparks, arbitrary count, roles, ranks, fabric edges, reusable content, transfers, build/install disk, unified memory, safety margins, and blockers. Accepting a mapping never implicitly starts a run.
...
On one, two, and three compatible Sparks, record build isolation, one exact OCI digest, disk peak, unified-memory admission, deterministic ranks, fabric edges, group readiness, route publication, rank-failure withdrawal, and rerun from retained content.
