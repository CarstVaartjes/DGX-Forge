...
**Goal:** Let an administrator select a local recipe revision, prove whether it fits one or more GPU nodes, install it with exact disk accounting, run it with live memory/topology checks, and understand the cluster from one maintenance UI.
...
- Create: `control/src/vonk_control/admission_models.py`
...
**Files:**
- Create: `control/src/vonk_control/install_admission.py`
- Create: `control/src/vonk_control/artifact_sizes.py`
...
**Files:**
- Create: `control/src/vonk_control/run_admission.py`
- Create: `control/src/vonk_control/topology.py`
- Create: `control/tests/test_run_admission.py`
- Create: `control/tests/test_topology.py`
- Modify: `control/src/vonk_control/package_compatibility.py`

- [ ] Write failing tests for recipe memory floor, observed model allocation, configured safety margin, other active reservations, stale inventory, missing runtime/plugin capability, occupied port, mixed GPU node hardware, rank count, tensor/pipeline parallel divisibility, worker fabric reachability, and entrypoint selection.
...
**Files:**
- Create: `control/src/vonk_control/recipe_operations.py`
- Create: `control/src/vonk_control/recipe_operation_worker.py`
- Create: `control/src/vonk_control/recipe_api.py`
...
**Files:**
- Create: `control/src/vonk_control/recipe_routes.py`
...
- [ ] Make LiteLLM call the validated GPU node entrypoint directly on the restricted management LAN; never add worker ranks as independent upstreams.
...
## Task 7: Build the GPU node cluster maintenance view

**Files:**
- Create: `control/src/vonk_control/cluster_view.py`
...
- [ ] Write backend and React tests first for one row/card per GPU node, online/stale/offline state, hardware capacity, free/reserved/observed memory, disk capacity, installed model artifacts with sizes/digests/refcounts, running models with RAM reservations and observed use, multi-node grouping, rank/entrypoint labels, and orphaned artifacts/processes.
...
- [ ] Commit: `feat(web): add GPU node cluster maintenance view`
...
- [ ] Run the same critical scenarios on physical GPU nodes. Capture before/after inventory, operation groups, reservations, container identities, route config, and an inference request through Tailscale/Caddy/LiteLLM.
