...
**Architecture:** A canonical release manifest binds control images, migrations, agent slots, node tooling, SBOMs, signatures, and protocol ranges. Host-local offline tooling updates the control plane; the online orchestrator updates healthy agents in topology-aware batches.

**Tech Stack:** Python 3.12, JSON Schema, OCI images, Docker Compose, cosign-compatible signatures, Alembic, systemd A/B supervisor, pytest
...
**Interfaces:**
- Produces `PlatformRelease.load(path)`, `verify_signature(trust)`, `compatibility(current)`, and canonical `digest`.
...
Test floating image tags, missing digest/signature/SBOM, overlapping architecture
entries, invalid protocol interval, destructive migration without predecessor
compatibility, unknown fields, and canonical digest under reordered input.
...
Require full SHA-256 digests, OCI `@sha256`, semantic version, Ed25519 signature
metadata, supported architectures, `control_api_protocol` and `agent_protocol`
closed intervals, migration expand revision and optional later contract
revision, plus recovery-compatible predecessor digests. Canonicalize before
signature verification and never resolve mutable tags.
...
```bash
git add schemas/platform-update-manifest.schema.json src/spark_profiles/schemas/platform-update-manifest.schema.json src/spark_profiles/platform_release.py pyproject.toml tests/spark_profiles/test_platform_release.py
...
Require exclusive offline lock and stopped worker/API proof. Verify release and
...
Download/verify into inactive temp slot, fsync, atomic rename, persist activation
