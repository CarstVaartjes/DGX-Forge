...
- [x] **Step 7: Build and publish from Spark 1**
...
git diff --check
set +e
uv run --no-project --with jsonschema -- bin/sparkctl validate agent-single --json
test $? -eq 3
set -e
```

Expected: the full suite passes; `sparkctl validate agent-single` resolves a
...
- [x] **Step 1: Deploy the immutable adapter only to Spark 1**
...
uvx --from ruff==0.16.1 ruff check .
git diff --check
ssh dgx-spark-1 curl --fail --silent http://127.0.0.1:8888/v1/models
```

Expected: all repository gates pass and the restored Mia endpoint advertises
`deepseek`. The final curl runs on Spark 1 so its loopback address reaches the
restored Mia head directly; a controller-side check instead requires the SSH
tunnel documented in `docs/runbooks/sparkctl.md`. Run the complete CI matrix
after push.
