# DS4 DeepSeek Flash 0731 Single-GPU node Runtime Implementation Plan
...
**Goal:** Add a pinned, containerized, single-GPU node DS4 serving lane for
...
- The OpenAI model name is always `deepseek`. Clients never select `single`,
  `lite`, `DS4`, the quantization, or the physical GPU node.
- Initial placement is fixed to GPU node 1 as `single-exclusive`; GPU node 2 remains
  idle. A GPU node-2 copy is a separate content-addressed definition, not live
  migration.
- Start with a 32,768-token context. Larger context and co-residency are
  ...
  initial one-GPU node admission limit.
...
- Modify: `src/cluster_profiles/schemas/model-definition-evidence.schema.json`
...
- [x] **Step 1: Add failing one-node evidence tests**

Add catalog tests that construct a valid `single` definition on `node1` and a
prepared/verified evidence chain containing only GPU node 1. Assert that it loads.
Add negative cases for missing GPU node 1, an extra GPU node 2, and a single-node
report attached to the existing dual definition. Keep the existing Mia report
fixtures byte-for-byte valid.

- [x] **Step 2: Add failing workload-scoped deployment tests**

Extend the deployer fixture with `nodes = ["node1"]` and assert dry-run and
apply invoke only `vonk-node-1`. Retain the distributed test that invokes
`vonk-node-1` and `vonk-node-2`.

- [x] **Step 3: Run the focused tests and observe the failures**

Run:

```bash
uv run pytest tests/cluster_profiles/test_catalog.py \
...
- [x] **Step 4: Implement topology-aware evidence validation**
...
- [x] **Step 5: Implement workload-scoped release deployment**

In `_load_release`, validate `workload["nodes"]` as a nonempty unique subset of
`{"node1", "node2"}`. Resolve aliases in declared-node order from
...
- [x] **Step 6: Verify schema synchronization and focused behavior**

Run:

```bash
cmp schemas/model-definition-evidence.schema.json \
  src/cluster_profiles/schemas/model-definition-evidence.schema.json
uv run pytest tests/cluster_profiles/test_catalog.py \
  tests/scripts/test_deploy_runtime_release.py -v
uvx --from ruff==0.16.1 ruff check src tests scripts
git diff --check
```
...
```bash
git add schemas src/cluster_profiles/schemas src/cluster_profiles/catalog.py \
  scripts/deploy-runtime-release tests/cluster_profiles/test_catalog.py \
...
- [x] **Step 1: Write failing manifest-verifier tests**
...
- [x] **Step 2: Run the tests and observe the import failure**
...
- [x] **Step 3: Implement the strict verifier and checked manifest**
...
- [x] **Step 4: Write the immutable upstream audit**
...
the v0.5.3 loader and does not fit one GPU node.
...
- [x] **Step 6: Verify documentation contracts and manifest behavior**

```bash
uv run pytest tests/adapters/test_ds4_artifact_manifest.py \
  tests/cluster_profiles/test_contracts.py -v
uvx --from ruff==0.16.1 ruff check adapters/deepseek/ds4/tools tests
git diff --check
```
...
git commit -m "docs: pin the DS4 GPU node serving lane"
git push origin main
```
...
- [x] **Step 1: Write failing static render and pin tests**
...
- [x] **Step 2: Write failing served-name patch tests**
...
- [x] **Step 4: Implement the multi-stage image and minimal alias patch**
...
- [x] **Step 5: Implement the loopback-only Compose service**
...
```bash
uv run pytest tests/adapters/test_ds4_runtime.py -v
DS4_MODELS_GID=1000 docker compose -f adapters/deepseek/ds4/compose.yaml \
  --env-file adapters/deepseek/ds4/config/runtime.env config --quiet
uvx --from ruff==0.16.1 ruff check tests
git diff --check
```
...
- [x] **Step 7: Build and publish from GPU node 1**
...
git commit -m "feat: build the DS4 GPU node runtime"
git push origin main
```
...
- Modify: `tests/cluster_profiles/test_catalog.py`
- Modify: `tests/cluster_profiles/test_admission.py`
- Modify: `tests/cluster_profiles/test_switcher.py`

**Interfaces:**
- Produces: adapter operations `prepare`, `verify`, `start`, `health`, `infer`,
  `stop`, and `verify-release`, each running on GPU node 1 without a role suffix.
...
Cover exact-node enforcement (`node-3542` only), free-memory and free-disk
...
- [x] **Step 2: Write failing catalog/admission/switch tests**

Assert the definition is locked and planned, `agent-single` places it only on
GPU node 1 with GPU node 2 empty, admission refuses it until exact accepted evidence
exists, endpoint name remains `deepseek`, single-node lifecycle calls never
touch GPU node 2, and no `head`/`worker` argument is appended.

- [x] **Step 3: Run focused tests and observe failures**

```bash
uv run pytest tests/adapters/test_ds4_runtime.py \
  tests/cluster_profiles/test_catalog.py \
  tests/cluster_profiles/test_admission.py \
  tests/cluster_profiles/test_switcher.py -v
```

Expected: FAIL because the adapter, definition, and profile are absent.

- [x] **Step 4: Implement the fail-closed adapter**
...
- [x] **Step 5: Add the content-addressed definition and profile intent**

Use the pushed image digest from Task 3 and the generated runtime-manifest
digest. Define GPU node 1 only, `topology = "single"`,
...
- [x] **Step 6: Generate the immutable adapter manifest**
...
- [x] **Step 7: Verify all local framework behavior**
...
git diff --check
set +e
uv run --no-project --with jsonschema -- bin/vonkctl validate agent-single --json
test $? -eq 3
set -e
```

Expected: the full suite passes; `vonkctl validate agent-single` resolves a
valid profile and exits 3 because admission reports verified maturity and no
...
git commit -m "feat: add the DS4 single-GPU node definition"
git push origin main
```

### Task 5: Deploy, Prepare, and Verify the DS4 Lane on GPU node 1
...
**Interfaces:**
- Produces: a deployed immutable adapter and complete verified cache on GPU node 1.
- Produces: maturity `verified` plus an operational report with `accepted: false`.

- [x] **Step 1: Deploy the immutable adapter only to GPU node 1**

Run `scripts/deploy-runtime-release deepseek-agent-single` in its default
dry-run mode, confirm its only alias is `vonk-node-1`, then apply. Verify the
...
- [x] **Step 2: Prepare the artifact cache while Mia remains live**

Run the DS4 adapter's `prepare` operation on GPU node 1. Monitor both file sizes
...
- [x] **Step 3: Verify offline readiness and record prepared evidence**
...
port 8888 and the single-GPU node memory budget. Generate the canonical prepared
report for GPU node 1's current boot ID, advance maturity from `planned` to
`prepared`, and verify the catalog.

- [x] **Step 4: Stop Mia safely and start DS4**

Stop the Mia head on GPU node 1, then its worker on GPU node 2. Require both nodes to
recover within the Mia memory tolerance. Run the DS4 adapter's full `verify`
operation from the immutable release, then start DS4 on GPU node 1, leaving GPU node
2 idle. If verification or DS4 fails, stop DS4 if necessary and leave the
cluster stopped; do not automatically restart Mia.

- [x] **Step 5: Run the live DS4 quality and identity gates**
...
- [x] **Step 6: Record verified-not-accepted evidence**
...
- [x] **Step 7: Restore the default Mia home runtime explicitly**

After all DS4 evidence is durably written, stop DS4 and invoke its unchanged
`verify-release` operation to require container and port cleanup plus GPU node 1
memory recovery. Start the Mia worker on GPU node 2 first, then the Mia head on
GPU node 1, and rerun its hardened health and `deepseek` identity checks. This is
an explicit successful-test restoration, not automatic rollback: any failure
leaves the cluster stopped for diagnosis.

- [x] **Step 8: Reconcile documentation**
...
- [x] **Step 9: Run final repository and live verification**

```bash
uv run pytest -q
uvx --from ruff==0.16.1 ruff check .
git diff --check
ssh vonk-node-1 curl --fail --silent http://127.0.0.1:8888/v1/models
```

Expected: all repository gates pass and the restored Mia endpoint advertises
`deepseek`. The final curl runs on GPU node 1 so its loopback address reaches the
restored Mia head directly; a controller-side check instead requires the SSH
tunnel documented in `docs/runbooks/vonkctl.md`. Run the complete CI matrix
after push.
...
git commit -m "ops: verify the DS4 single-GPU node runtime"
git push origin main
...
on GPU node 1, its immutable evidence is pushed, and the default Mia dual-GPU node
