...
- Fitting individually does not imply that arbitrary models may remain loaded together. Every profile begins as exclusive and becomes shareable only after a pairwise co-residency test.
- The user-facing profile always selects the best accepted Spark-optimized path. Official generic paths remain non-serving correctness references and diagnostic fallbacks.

Each Spark has 128 GB of unified memory shared by the operating system, CPU, and GPU. Published VRAM requirements and checkpoint sizes therefore indicate feasibility, not the final admission limit. Cluster acceptance records measured resident weights, activations or KV cache, runtime workspaces, operating-system headroom, and recovered memory after shutdown.

## Official model versus Spark-optimized profile
...
| --- | --- | --- | --- | --- | --- |
| DeepSeek-V4-Flash-0731 | The [official checkpoint](https://huggingface.co/deepseek-ai/DeepSeek-V4-Flash-0731) snapshot is about 155.44 GiB of weights, before KV cache and runtime workspaces. | Audited [MiaAI-Lab dual-Spark recipe](https://github.com/MiaAI-Lab/DeepSeek-v4-Flash-DSpark-2x-DGX-Spark) using TP=2, DSpark MTP, and padded NVFP4 KV. [DS4 on Spark](https://github.com/Entrpi/ds4-on-spark) is a separate quantized single-node alternative. NVIDIA's [NVFP4 checkpoint](https://huggingface.co/nvidia/DeepSeek-V4-Flash-NVFP4) is not adopted until TP=2 is proven. | Both Sparks for the default service | `dual-exclusive` | Dual-Spark candidate is implementation-audit pending; live KV capacity must be measured. |
...
| Nemotron 3 Nano Omni 30B-A3B | NVIDIA publishes BF16 at 62 GB, FP8 at 33 GB, and NVFP4 at 21 GB; one Spark is supported. | Official [Nano Omni NVFP4](https://huggingface.co/nvidia/Nemotron-3-Nano-Omni-30B-A3B-Reasoning-NVFP4) through NVIDIA's [Spark vLLM playbook](https://build.nvidia.com/spark/vllm/instructions); FP8 remains a measured comparison lane. | One Spark | `single-exclusive`, then co-residency candidate | Official Spark path; multimodal and pairwise acceptance pending. |
...
Single-Spark profiles:
  Spark 1 [agent or creative profile]          Spark 2 [agent or creative profile]
           ^ one accepted profile initially              ^ one accepted profile initially
```

The likely long-term arrangement is a lightweight agent such as Nemotron Nano Omni or a DS4 DeepSeek lane on one Spark while the other Spark runs an image, 3D, or rigging profile. This is a target configuration, not an accepted combination. Each exact pair must pass combined startup, peak-memory, sustained inference, thermal, output-quality, and clean-shutdown tests before the controller permits it.

## What “fits” does and does not mean

`Fits on one Spark` means the official evidence and model scale are below one Spark's 128 GB unified-memory capacity with a credible runtime path. It does not yet mean:
...
- the model can coexist with another resident profile; or
