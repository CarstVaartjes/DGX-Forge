...
| Nemotron 3 Nano Omni 30B-A3B | NVIDIA publishes BF16 at 62 GB, FP8 at 33 GB, and NVFP4 at 21 GB; one GPU node is supported. | Official [Nano Omni NVFP4](https://huggingface.co/nvidia/Nemotron-3-Nano-Omni-30B-A3B-Reasoning-NVFP4) through NVIDIA's [GPU node vLLM playbook](https://build.nvidia.com/spark/vllm/instructions); FP8 remains a measured comparison definition. | One GPU node | `single-exclusive`, then co-residency candidate | Official GPU node path; multimodal and exact-set acceptance pending. |
...
| TRELLIS.2 4B | Official [TRELLIS.2](https://github.com/microsoft/TRELLIS.2) requires at least 24 GB of GPU memory. | Audited GPU node build informed by [vonk-trellis2](https://github.com/raziel2001au/dgx-trellis2) and [TRELLIS.2 Vonk Forge GPU node Docker](https://github.com/dr-vij/Trellis2-DGX-Spark-Docker). | One GPU node | `single-exclusive` initially | Official model fits by published requirement; community GPU node builds require audit and output-parity validation. |
