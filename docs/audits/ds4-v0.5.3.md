...
| Drafter | `bleysg/DeepSeek-V4-Flash-DSpark-drafter-GGUF` @ `81c6fdd38f9582da45ba27f0ed7b63bcd3ea3b62` | `DSpark-drafter-Q2K-Q8-0731.gguf` | 6,971,241,504 | `8fa269560dc76fd73e4233ad9b1938b5f65dd363381fd9b1a5c6183f7d12d686` |
...
| DS4-on-Spark | `https://github.com/Entrpi/ds4-on-spark.git@185487ba5749a3c24a71ca81d1bc514c45f10dca`, source archive SHA-256 `7db338d0a441fed36c5e4e7af44ff670e8bfe567e88d482f00ff6a3dc0e5dbe3` | MIT | Audited recipe only; never execute its installer as deployment automation. |
...
supported build is `make cuda-spark` for the pinned v0.5.3 source on the Spark
CUDA environment (GB10 `sm_121` with the Spark HBM weight cache); rebuild from
...
- Use mapped/registered no-copy startup. Never set `DS4_CUDA_COPY_MODEL` and
  never enable `DS4_MODEL_ANON_HUGE`; either can violate the one-Spark memory
...
`155,976,458,848` bytes, which exceeds one Spark's visible memory before KV
cache or runtime overhead. DS4 v0.5.3's loader rejects its GGUF type 39.
MXFP4 remains deferred until both loader support and measured one-Spark
admission exist.
