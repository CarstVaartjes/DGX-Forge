...
| LiteLLM/Caddy routes | Inference availability; shadow model/upstream | Routes only from eligible accepted snapshot, certificate-bound node ID, fresh policy-bounded management observation, and repository-declared workload port; LiteLLM consumes only the already-rendered string. Publication is atomic and address replacement enters maintenance first | Route-state alert and generation digests; retain maintenance on invalid replacement and prior generation only for non-transition apply failure | routes/LiteLLM tests |
...
| Tailscale gateway recovery | Human ingress identity; stolen OAuth secret or lost state | File-backed OAuth client limited to `auth_keys` for `tag:dgx-gateway`, persisted state, exact Service auto-approvals, no wildcard or LAN fallback | Revoke OAuth client/node/tag, verify service-host status, restore encrypted state or create one reviewed replacement | `deploy/compose/tests/test_tailscale.py`, Tailscale runbook |
| AI devbox SSH | Coding credentials and workspaces; hostile tailnet user or container escape | Separate SSH grant plus OpenSSH public key, `ai-dev` only, root/password/forwarding denied, read-only root, no socket/devices/private control networks | Strict persistent host keys, runtime isolation test, key rotation, encrypted backup and fingerprint recovery | `deploy/compose/tests/test_ai_devbox.py`, `ai-devbox-runtime.sh` |
