...
| Tailscale gateway recovery | Human ingress identity; stolen OAuth secret, lost state, or stale extra Service | File-backed OAuth client limited to `auth_keys` for `tag:dgx-gateway`, persisted state, exact Service auto-approvals, exact exported two-Service map, explicit HTTPS listener, no wildcard or LAN fallback | Revoke OAuth client/node/tag, verify status and exported service map, restore encrypted state or create one reviewed replacement | `deploy/compose/tests/test_tailscale.py`, Tailscale runbook |
