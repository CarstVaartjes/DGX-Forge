...
class RecipeRun(Base):
    __tablename__ = "recipe_runs"; __table_args__ = (CheckConstraint(_lower_hex("plan_digest", 64), name="ck_recipe_runs_digest"), CheckConstraint("state IN ('planned','starting','running','stopping','stopped','failed','lost')", name="ck_recipe_runs_state"), CheckConstraint("route_state IN ('withdrawn','pending','published','failed')", name="ck_recipe_runs_route_state"), CheckConstraint("route_generation IS NULL OR route_generation>=1", name="ck_recipe_runs_route_generation"), CheckConstraint("route_digest IS NULL OR length(route_digest)=64", name="ck_recipe_runs_route_digest"))
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4())); installation_id: Mapped[str] = mapped_column(ForeignKey("recipe_installations.id", ondelete="RESTRICT"), nullable=False, index=True); alias: Mapped[str] = mapped_column(String(128), nullable=False); plan_digest: Mapped[str] = mapped_column(String(64), nullable=False, unique=True); plan: Mapped[dict[str, object]] = mapped_column(JSON, nullable=False); state: Mapped[str] = mapped_column(String(24), nullable=False); route_state: Mapped[str] = mapped_column(String(24), nullable=False, default="withdrawn", server_default="withdrawn"); route_generation: Mapped[int | None] = mapped_column(BigInteger); route_digest: Mapped[str | None] = mapped_column(String(64)); route_error: Mapped[str | None] = mapped_column(String(512)); actor: Mapped[str] = mapped_column(String(200), nullable=False); created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False); updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False); stopped_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
