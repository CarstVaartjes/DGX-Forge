"""Explicit test-only compatibility handlers for pre-agent direct execution."""
...
LEGACY_DIRECT_TRANSPORT = "explicit-test-only"


class LegacyRuntimeHandlers:
    def __init__(
        self,
        ...
        *,
        ...
        if authorization != LEGACY_DIRECT_TRANSPORT:
            raise RuntimeError("legacy direct transport requires explicit-test-only authorization")
    ...
    def probe(self, request: HandlerRequest) -> Mapping[str, object]:
        ...
        return self._run((str(self._root / "bin/vonkctl"), "nodes", "status", "--json"))
    ...
    def reconcile(self, request: HandlerRequest) -> Mapping[str, object]:
        ...
        for workload in sorted(workloads):
            ...
            commands += 1
        self._run((str(self._root / "bin/vonkctl"), "switch", profile, "--json"))
        if self._route_manager is not None:
