...
def upgrade() -> None:
    ...
    op.create_index("ix_node_artifacts_node_id", "node_artifacts", ["node_id"])
    op.create_table("recipe_installations",
        sa.Column("id", sa.String(36), primary_key=True), sa.Column("recipe_revision_id", sa.String(36), sa.ForeignKey("local_recipe_revisions.id", ondelete="RESTRICT"), nullable=False), sa.Column("mapping_id", sa.String(36), sa.ForeignKey("cluster_mappings.id", ondelete="RESTRICT"), nullable=False), sa.Column("mapping_generation", sa.Integer, nullable=False), sa.Column("recipe_build_id", sa.String(36), sa.ForeignKey("recipe_builds.id", ondelete="RESTRICT"), nullable=False), sa.Column("image_digest", sa.String(71), nullable=False), sa.Column("plan_digest", sa.String(64), nullable=False, unique=True), sa.Column("plan", sa.JSON, nullable=False),
    ...
    op.create_table("installation_nodes",
        ...
        sa.Column("rank", sa.Integer, nullable=False), sa.Column("role", sa.String(64), nullable=False), sa.Column("state", sa.String(24), nullable=False), sa.Column("required_bytes", sa.BigInteger, nullable=False), sa.Column("installed_bytes", sa.BigInteger, nullable=False, server_default="0"), sa.Column("evidence_digest", sa.String(64)), sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
    ...
    op.create_index("ix_installation_nodes_installation_id", "installation_nodes", ["installation_id"])
    op.create_index("ix_installation_nodes_node_id", "installation_nodes", ["node_id"])
    op.create_index("ix_recipe_installations_recipe_revision_id", "recipe_installations", ["recipe_revision_id"])
    op.create_index("ix_recipe_installations_mapping_id", "recipe_installations", ["mapping_id"])
    op.create_index("ix_recipe_installations_recipe_build_id", "recipe_installations", ["recipe_build_id"])
    op.create_table("recipe_runs",
        sa.Column("id", sa.String(36), primary_key=True), sa.Column("installation_id", sa.String(36), sa.ForeignKey("recipe_installations.id", ondelete="RESTRICT"), nullable=False), sa.Column("mapping_id", sa.String(36), sa.ForeignKey("cluster_mappings.id", ondelete="RESTRICT"), nullable=False), sa.Column("mapping_generation", sa.Integer, nullable=False), sa.Column("alias", sa.String(128), nullable=False), sa.Column("plan_digest", sa.String(64), nullable=False, unique=True), sa.Column("plan", sa.JSON, nullable=False),
        sa.Column("state", sa.String(24), nullable=False), sa.Column("route_state", sa.String(24), nullable=False, server_default="withdrawn"), sa.Column("route_generation", sa.BigInteger), sa.Column("route_digest", sa.String(64)), sa.Column("route_error", sa.String(512)), sa.Column("actor", sa.String(200), nullable=False), sa.Column("created_at", sa.DateTime(timezone=True), nullable=False), sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False), sa.Column("stopped_at", sa.DateTime(timezone=True)),
        sa.CheckConstraint(digest("plan_digest"), name="ck_recipe_runs_digest"), sa.CheckConstraint("state IN ('planned','starting','running','stopping','stopped','failed','lost')", name="ck_recipe_runs_state"), sa.CheckConstraint("route_state IN ('withdrawn','pending','published','failed')", name="ck_recipe_runs_route_state"), sa.CheckConstraint("route_generation IS NULL OR route_generation>=1", name="ck_recipe_runs_route_generation"), sa.CheckConstraint("route_digest IS NULL OR length(route_digest)=64", name="ck_recipe_runs_route_digest"))
    op.create_index("ix_recipe_runs_installation_id", "recipe_runs", ["installation_id"])
    op.create_index("ix_recipe_runs_mapping_id", "recipe_runs", ["mapping_id"])
    op.create_table("run_nodes",
        ...
        sa.Column("rank", sa.Integer, nullable=False), sa.Column("role", sa.String(64), nullable=False), sa.Column("state", sa.String(24), nullable=False), sa.Column("port", sa.Integer, nullable=False),
        ...
        sa.CheckConstraint("length(role) BETWEEN 1 AND 64", name="ck_run_nodes_role"), sa.UniqueConstraint("run_id", "node_id", name="uq_run_node"), sa.UniqueConstraint("run_id", "rank", name="uq_run_rank"))
    op.create_index("ix_run_nodes_run_id", "run_nodes", ["run_id"])
    op.create_index("ix_run_nodes_node_id", "run_nodes", ["node_id"])
    op.create_table("resource_reservations",
        ...
        sa.CheckConstraint("kind IN ('disk','unified-memory','host-memory','gpu-memory','port')", name="ck_reservations_kind"), sa.CheckConstraint("state IN ('active','released','expired') AND amount_bytes>=0", name="ck_reservations_state"), sa.CheckConstraint(digest("plan_digest"), name="ck_reservations_digest"))
