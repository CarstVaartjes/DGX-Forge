...
/**
 * Spark-local package inventory and storage safety controls.
 *
...
export function PackageInventory({api}: {api: InventoryApi}) {
  ...
  async function refresh() {
    ...
    catch (value) { setError(value instanceof Error ? value.message : "Unable to load Spark package inventory"); }
  }
  ...
  if (!api.packageInventory) return <section className="inventory-shell review-card" aria-labelledby="inventory-unavailable-heading">
    <h3 id="inventory-unavailable-heading">Spark package inventory</h3>
    <p>The control plane is still wiring Spark inventory telemetry. Downloads and running services remain separate; once telemetry is available this view will show local generations, free space, and safe cleanup previews.</p>
  </section>;

  return <section className="inventory-shell" aria-labelledby="inventory-heading">
    <div className="page-heading inventory-heading"><div><h3 id="inventory-heading">Spark package inventory</h3><p>Downloaded packages are local and resumable. Removing or collecting a generation never interrupts an active or leased service.</p></div><button type="button" onClick={() => void refresh()} disabled={busy}>Refresh</button></div>
    {error && <p role="alert">{bounded(error)}</p>}
    <div className="inventory-summary" aria-label="Cluster package storage summary">
      <div><span>Used</span><strong>{bytes(totals.used)}</strong></div><div><span>Available</span><strong>{bytes(totals.available)}</strong></div><div><span>Spark capacity</span><strong>{bytes(totals.total)}</strong></div><div><span>Nodes reporting</span><strong>{nodes.length}</strong></div>
    </div>
    {nodes.length === 0 && <p>No Spark package inventory is reporting yet.</p>}
      ...
      return <article className="inventory-node" key={node.node_id} aria-labelledby={`inventory-node-${node.node_id}`}>
        ...
        <div className="inventory-packages">{(node.packages ?? []).length === 0 && <p>No package generations installed.</p>}{(node.packages ?? []).map(entry => {
          ...
          return <div className="inventory-package" key={`${node.node_id}:${entry.deployment_id}:${entry.release_digest}:${entry.content_group}`}>
            ...
            <div className="package-resources"><span>Disk <strong>{bytes(entry.installed_bytes)}</strong></span><span>Run memory <strong>{bytes(entry.resources.host_memory_bytes)}</strong></span><span>GPU memory <strong>{bytes(entry.resources.gpu_memory_bytes)}</strong></span><span>KV base <strong>{bytes(entry.resources.kv_cache_base_bytes)}</strong></span><span>Sparks <strong>{entry.resources.required_sparks}</strong></span></div>
