    ...
    {candidate && <>
      <section className="review-card" aria-label="Package candidate state">
        ...
        <dl className="evidence-grid compact">
          ...
          <div><dt>Lock digest</dt><dd><code>{bounded(candidate.lock?.digest)}</code></dd></div><div><dt>Provenance</dt><dd>{bounded(candidate.lock?.provenance)}</dd></div><div><dt>Compatibility</dt><dd>{candidate.compatibility.compatible.length} compatible; {candidate.compatibility.incompatible_count} incompatible</dd></div>
        </dl>
        <p>Components: {candidate.lock?.components.map(bounded).join(", ") || "—"}</p>
        <p>Dependencies: {candidate.lock?.dependencies.map(bounded).join(", ") || "—"}</p>
      </section>
      <section aria-labelledby="validation-heading"><h3 id="validation-heading">Validation</h3><ul>{candidate.validations.map((validation, index) => <li key={`${validation.backend}-${index}`}>{bounded(validation.backend)} — {bounded(validation.state)}{validation.reason_code ? `: ${bounded(validation.reason_code)}` : ""}</li>)}</ul></section>
      ...
      <section aria-labelledby="package-audit-heading"><h3 id="package-audit-heading">Audit evidence</h3><ul>{candidate.audit.map(event => <li key={event.request_id}>{bounded(event.action)} <code>{bounded(event.request_id)}</code></li>)}</ul></section>
    </>}
  </>;
}
