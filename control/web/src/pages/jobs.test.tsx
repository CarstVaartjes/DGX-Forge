...
const job: Job = {
  ...
  operation_next_cursor: null, operation_total: 1,
  ...
  target_next_cursor: null, target_total: 1,
};
...
it("invalidates stale detail and refetches authority after a resume conflict", async () => {
  const detailCalls: string[] = [];
  const api = {
    jobs: async () => ({jobs: [{id: jobId, kind: "reconcile", state: "waiting-for-operator"}], next_cursor: null, total: 1}),
    job: async (id: string) => {
      detailCalls.push(id);
      return detailCalls.length === 1 ? job : {...job, state: "queued", status_reason: null};
    },
    resumeJob: async () => { throw new Error("Control API returned 409"); },
  } as unknown as ControlApi;
  render(<JobsPage api={api}/>);
  const user = userEvent.setup();
  ...
  expect(await screen.findByText(/Control API returned 409/)).toBeVisible();
  expect(detailCalls).toEqual([jobId, jobId]);
  expect(screen.queryByRole("button", {name: "Resume job"})).not.toBeInTheDocument();
  expect(screen.getAllByText("queued")).toHaveLength(2);
});
it("follows bounded server cursors so every job and operation remains reachable", async () => {
  ...
  const jobCalls: (string | undefined)[] = [];
  const operationCalls: (string | undefined)[] = [];
  const api = {
    jobs: async (cursor?: string) => {
      jobCalls.push(cursor);
      return cursor === "jobs-2"
        ? {jobs: jobs.slice(20), next_cursor: null, total: 23}
        : {jobs: jobs.slice(0, 20), next_cursor: "jobs-2", total: 23};
    },
    job: async (id: string, operationCursor?: string) => {
      operationCalls.push(operationCursor);
      return operationCursor === "operations-2"
        ? {...job, id, operations: operations.slice(20), operation_next_cursor: null, operation_total: 23, state: "running", target_next_cursor: null, target_total: 1}
        : {...job, id, operations: operations.slice(0, 20), operation_next_cursor: "operations-2", operation_total: 23, state: "running", target_next_cursor: null, target_total: 1};
    },
  } as unknown as ControlApi;
  render(<JobsPage api={api}/>);
  const user = userEvent.setup();
  ...
  expect(await screen.findByText("job-22")).toBeVisible();
  ...
  expect(await screen.findByText("operation-22")).toBeVisible();
  expect(jobCalls).toEqual([undefined, "jobs-2"]);
  expect(operationCalls).toEqual([undefined, "operations-2"]);
});
