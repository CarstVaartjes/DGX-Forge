...
import type {WorkloadRunApi, WorkloadRunPreview} from "../api/types";
import {ImportReport} from "../components/import-report";

export function WorkloadRunImportPage({api}: {api: WorkloadRunApi}) {
  const [source, setSource] = useState(""); const [preview, setPreview] = useState<WorkloadRunPreview | null>(null);
  ...
  async function showPreview() {
    ...
    try { setPreview(await api.previewWorkloadRun(source)); } catch (value) { setError(value instanceof Error ? value.message.slice(0, 256) : "Unable to preview import"); }
    finally { setBusy(false); }
  }
  async function apply() {
    if (!preview) return; setBusy(true); setError("");
    try { const result = await api.applyWorkloadRun(source, preview.source_sha256, preview.report_digest); setMessage(`Imported ${result.lifecycle} draft revision ${result.revision_number}. Open it from the catalog to provide missing information.`); }
    catch (value) { setError(value instanceof Error ? value.message.slice(0, 256) : "Unable to import draft"); }
    finally { setBusy(false); }
  }
  return <>
    <div className="page-heading"><div><h2>Import WorkloadRun recipe</h2><p>Preview is non-executing. Every source field is explained before anything is saved.</p></div></div>
    {error && <p role="alert">{error}</p>}{message && <p role="status">{message}</p>}
    <section className="import-source"><label>Upload YAML<input type="file" accept=".yaml,.yml,application/yaml,text/yaml" onChange={event => void loadFile(event.target.files?.[0])}/></label><span>or</span><label>WorkloadRun YAML<textarea rows={16} value={source} onChange={event => {setSource(event.target.value); setPreview(null);}} maxLength={262144}/></label><button onClick={() => void showPreview()} disabled={busy || !source}>Preview import</button></section>
