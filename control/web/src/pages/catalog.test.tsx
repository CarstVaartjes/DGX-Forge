...
test("separates local, WorkloadRun, and global recipe origins", async () => {
  ...
  expect(screen.getByText("2 nodes, 3 nodes, 4 nodes")).toBeVisible();
...
test("reviews an exact public revision before importing it locally", async () => {
  ...
  const api = {
    ...
    previewGlobalRecipe: async (value: string) => ({
      ...
      document: {metadata: {title: "Qwen3"}, build: {context: {sha256: "f".repeat(64)}, dockerfile: "Dockerfile"}, deployment_profiles: [{name: "solo", node_count: 1, roles: [{resources: {disk: {image_bytes: 5_000_000_000, artifact_bytes: 53_000_000_000, staging_bytes: 8_000_000_000, cache_bytes: 0, rollback_bytes: 0, safety_margin_bytes: 0}, memory: {startup_peak_bytes: 72_000_000_000}}}]}]},
    }),
