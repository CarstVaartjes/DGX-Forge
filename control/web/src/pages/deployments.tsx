...
export function DeploymentsPage({api}: {api: Pick<PackageApi, "deployments" | "previewPackageRollout" | "startPackageRollout" | "packageRollout" | "previewPackageRollback" | "rollbackPackage">}) {
  ...
  async function startRollout() { if (!selected || !rolloutPreview || rolloutConfirmation !== rolloutPreview.digest) return; const started = await api.startPackageRollout(selected.id, rolloutPreview.digest); setRollout(await api.packageRollout(started.id)); }
  async function previewRollback(deployment: PackageDeployment) { setSelected(deployment); setRollbackPreview(await api.previewPackageRollback(deployment.id)); setRollbackConfirmation(""); }
  async function rollback() { if (!selected || !rollbackPreview || rollbackConfirmation !== rollbackPreview.digest) return; await api.rollbackPackage(selected.id, rollbackPreview.digest); }
