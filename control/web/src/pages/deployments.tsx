...
export function DeploymentsPage({api}: {api: Pick<PackageApi, "deployments" | "previewPackageRollout" | "startPackageRollout" | "packageRollout" | "previewPackageRollback" | "rollbackPackage">}) {
  ...
  return <>
    ...
    {rolloutPreview && <section className="review-card" aria-label="Package rollout preview"><h3>Exact rollout preview</h3><p><code>{rolloutPreview.digest}</code></p><p>{gibibytes(rolloutPreview.download_remaining_bytes)} remaining; {gibibytes(rolloutPreview.storage_required_bytes)} storage required.</p>{rolloutPreview.resource_envelope && <p>Placement: {rolloutPreview.resource_envelope.required_sparks} Spark(s), {rolloutPreview.resource_envelope.topology}. Per-Spark: {gibibytes(rolloutPreview.resource_envelope.per_node.host_memory_bytes)} host memory; {gibibytes(rolloutPreview.resource_envelope.per_node.gpu_memory_bytes)} GPU memory; {gibibytes(rolloutPreview.resource_envelope.per_node.kv_cache_base_bytes)} KV base + {gibibytes(rolloutPreview.resource_envelope.per_node.kv_cache_per_token_bytes)} per token.</p>}<p>Canary: {rolloutPreview.canary.map(bounded).join(", ") || "—"}</p><p>Offline pending: {rolloutPreview.offline_pending.map(bounded).join(", ") || "None"}</p><label>Type the exact rollout preview digest<input value={rolloutConfirmation} onChange={event => setRolloutConfirmation(event.target.value)} maxLength={512}/></label><button type="button" disabled={rolloutConfirmation !== rolloutPreview.digest} onClick={() => void startRollout()}>Start exact rollout</button></section>}
