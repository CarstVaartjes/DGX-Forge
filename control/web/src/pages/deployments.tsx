...
export function DeploymentsPage({api}: {api: Pick<PackageApi, "deployments" | "previewPackageRollout" | "startPackageRollout" | "packageRollout" | "previewPackageRollback" | "rollbackPackage">}) {
  ...
  return <>
    ...
    {rolloutPreview && <section className="review-card" aria-label="Package rollout preview"><h3>Exact rollout preview</h3><p><code>{rolloutPreview.digest}</code></p><p>{gibibytes(rolloutPreview.download_remaining_bytes)} remaining; {gibibytes(rolloutPreview.storage_required_bytes)} storage required.</p>{rolloutPreview.resource_envelope && <><p>Placement: {rolloutPreview.resource_envelope.required_nodes} GPU node(s), {rolloutPreview.resource_envelope.topology}, world size {rolloutPreview.resource_envelope.world_size}. Fabric: {rolloutPreview.resource_envelope.fabric.kind} ({rolloutPreview.resource_envelope.fabric.min_bandwidth_mbps} Mbps minimum).</p><p>Per-GPU node memory: {gibibytes(rolloutPreview.resource_envelope.per_node.host_memory_bytes)} total = {gibibytes(rolloutPreview.resource_envelope.per_node.resident_memory_bytes)} resident + {gibibytes(rolloutPreview.resource_envelope.per_node.auxiliary_memory_bytes)} auxiliary + {gibibytes(rolloutPreview.resource_envelope.per_node.activation_memory_bytes)} activation + {gibibytes(rolloutPreview.resource_envelope.per_node.workspace_memory_bytes)} workspace; {gibibytes(rolloutPreview.resource_envelope.per_node.gpu_memory_bytes)} GPU memory; {gibibytes(rolloutPreview.resource_envelope.per_node.kv_cache_base_bytes)} KV base + {gibibytes(rolloutPreview.resource_envelope.per_node.kv_cache_per_token_bytes)} per token.</p><p>Ranks: {rolloutPreview.resource_envelope.ranks.map(rank => `${rank.rank}:${rank.role}`).join(", ") || "—"}</p></>}<p>Canary: {rolloutPreview.canary.map(bounded).join(", ") || "—"}</p><p>Offline pending: {rolloutPreview.offline_pending.map(bounded).join(", ") || "None"}</p><label>Type the exact rollout preview digest<input value={rolloutConfirmation} onChange={event => setRolloutConfirmation(event.target.value)} maxLength={512}/></label><button type="button" disabled={rolloutConfirmation !== rolloutPreview.digest} onClick={() => void startRollout()}>Start exact rollout</button></section>}
