...
function recipeDocument(fields: Fields, sourceSha256: string, archiveBytes: number): CatalogRecipeDocument {
  ...
  return {
    ...
    deployment_profiles: [{
      name: nodes === 1 ? "solo" : `${nodes}-spark`, description: nodes === 1 ? "One Spark." : `${nodes} connected Sparks.`, node_count: nodes,
...
export function RecipeEditorPage({api, recipeId}: {api: EditorApi; recipeId?: string}) {
  ...
  return <>
    <div className="page-heading"><div><h2>Create local recipe</h2><p>Start from a source template, import SparkRun, or describe a fully custom OCI build. No prebuilt image is required.</p></div><a className="button" href="/catalog/import/sparkrun">Import SparkRun instead</a></div>
    {error && <p role="alert">{error}</p>}{message && <p role="status">{message}</p>}
