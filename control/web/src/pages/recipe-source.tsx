...
export function RecipeSourcePage({api, recipeId}: {api: SourceApi; recipeId: string}) {
  ...
  async function build() {
    if (!plan) return;
    setError("");
    try { const operation = await api.buildRecipe(plan); setMessage(`Build ${operation.owner_id} queued on ${operation.nodes.join(", ")}. The GPU node repeats the source gate before Podman starts.`); setPlan(null); }
    catch (value) { setError(value instanceof Error ? value.message.slice(0, 256) : "Unable to schedule build"); }
  }
  return <>
    <div className="page-heading"><div><h2>Security check &amp; build</h2><p>{recipe?.title ?? "Recipe"}: inspect locally, then send the exact digest to one selected builder GPU node.</p></div><div className="actions">{recipe && <a className="button" href={`/api/v1/catalog/source-bundles/${recipe.source_bundle_sha256}`} download>Download source tar</a>}<a className="button" href={`/catalog/${recipeId}`}>Back to recipe</a></div></div>
