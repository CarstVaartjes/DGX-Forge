...
import {WorkloadRunImportPage} from "./workload-run-import";
...
test("previews every field before hash-bound import", async () => {
  const calls: string[] = [];
  const api = {
    previewWorkloadRun: async (source: string) => { calls.push(`preview:${source}`); return {draft_document: {}, report, source_sha256: "a".repeat(64), report_digest: "b".repeat(64), redacted_source: {}, runnable: false}; },
    applyWorkloadRun: async (source: string, sourceHash: string, reportHash: string) => { calls.push(`apply:${sourceHash}:${reportHash}`); return {recipe_id: "10000000-0000-4000-8000-000000000001", revision_number: 1, lifecycle: "blocked"}; },
  };
  render(<WorkloadRunImportPage api={api}/>); const user = userEvent.setup();

  await user.type(screen.getByLabelText("WorkloadRun YAML"), "model: Example/Model");
