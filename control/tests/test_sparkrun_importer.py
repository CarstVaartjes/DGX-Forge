...
def test_every_source_leaf_has_exactly_one_report_item() -> None:
    ...
    reported_source_paths = [item.source_path for item in result.report if not item.source_path.startswith("/@missing/")]
...
def test_import_is_deterministic_and_explains_missing_requirements() -> None:
    ...
    assert any(item.source_path == "/@missing/resources" and item.disposition is ImportDisposition.OVERLAY_REQUIRED for item in first.report)
    assert any(item.source_path == "/container" and item.disposition is ImportDisposition.RESOLUTION_REQUIRED for item in first.report)
