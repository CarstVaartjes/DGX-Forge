...
def test_code_host_rejects_force_update_and_duplicate_branch_content() -> None:
    ...
    commit = host.create_change("vonk-control/one", "base", b"patch", "message", signed=True)
    assert host.create_change("vonk-control/one", "base", b"patch", "message", signed=True) == commit
    try:
        host.create_change("vonk-control/one", "other", b"different", "message", signed=True)
