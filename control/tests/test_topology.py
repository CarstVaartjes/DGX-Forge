...
def test_multinode_topology_has_one_entrypoint_and_deterministic_ranks() -> None:
    ...
    result = validate_topology(multinode(), placements, {item.node_id: ("runtime.vonk.v1", "fabric.tcp.mbps.10000") for item in placements})
...
def test_invalid_rank_node_and_entrypoint_shapes_are_blocked(placements) -> None:
    with pytest.raises(TopologyError):
        validate_topology(multinode(), placements, {item.node_id: ("runtime.vonk.v1", "fabric.tcp.mbps.10000") for item in placements})
...
def test_missing_runtime_or_fabric_capability_is_blocking() -> None:
    ...
    with pytest.raises(TopologyError) as caught:
        validate_topology(multinode(), placements, {item.node_id: ("runtime.vonk.v1", "fabric.tcp.mbps.10") for item in placements})
