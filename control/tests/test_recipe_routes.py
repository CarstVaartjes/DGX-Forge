...
def setup(tmp_path: Path, *, ranks=2, stale=False, failed_rank=False, validate=lambda _: True):
    ...
    with sessions.begin() as session:
        session.add_all(
            AgentNode(node_id=node, state="active", architecture="linux-arm64", capabilities=[])
            for node in nodes
        )
        recipe = LocalRecipe(slug="qwen", title="Qwen", description="Qwen", source_kind="local", created_by="admin", created_at=NOW, updated_at=NOW)
        session.add(recipe); session.flush()
        revision = LocalRecipeRevision(recipe_id=recipe.id, revision_number=1, lifecycle="resolved", schema_version=1, document={}, content_sha256="a" * 64, created_by="admin", created_at=NOW)
        session.add(revision); session.flush()
        installation = RecipeInstallation(recipe_revision_id=revision.id, plan_digest="b" * 64, plan={}, state="installed", actor="admin", created_at=NOW, updated_at=NOW)
        session.add(installation); session.flush()
        session.add_all(InstallationNode(installation_id=installation.id, node_id=node, state="installed", required_bytes=1, installed_bytes=1, updated_at=NOW) for node in nodes)
        run = RecipeRun(installation_id=installation.id, alias="qwen", plan_digest="c" * 64, plan={}, state="running", actor="admin", created_at=NOW, updated_at=NOW)
...
def test_all_ranks_must_be_fresh_and_ready_but_only_entrypoint_is_routed(tmp_path: Path) -> None:
...
def test_stale_or_failed_rank_blocks_gang_publication(tmp_path: Path, stale: bool, failed: bool) -> None:
    service, _publisher, applied, run_id = setup(tmp_path, stale=stale, failed_rank=failed)
...
def test_withdraw_publishes_empty_generation_before_workload_stop(tmp_path: Path) -> None:
...
def test_atomic_adapter_keeps_caddy_routes_static_and_activates_litellm(tmp_path: Path) -> None:
    ...
    assert json.loads((directory / "litellm.json").read_text())["model_list"][0]["model_name"] == "qwen"
