...
def _write_config(tmp_path: Path, document: object, *, raw: bytes | None = None) -> Path:
...
def test_configuration_rejects_missing_or_unknown_fields(tmp_path: Path, mutate) -> None:
    document = _valid_document(tmp_path)
    mutate(document)
...
def test_configuration_reads_are_utf8_and_size_bounded(tmp_path: Path) -> None:
    ...
    with pytest.raises(AgentConfigError, match="large"):
        AgentConfig.load(_write_config(tmp_path, {}, raw=b"{" + b" " * (64 * 1024) + b"}"))


@pytest.mark.parametrize("field", ["certificate_path", "private_key_path", "ca_path", "installed_policy_path"])
def test_configuration_rejects_oversized_identity_or_policy_files(tmp_path: Path, field: str) -> None:
    document = _valid_document(tmp_path)
    oversized = tmp_path / f"large-{field}"
    oversized.write_bytes(b"x" * (64 * 1024 + 1))
    oversized.chmod(0o600 if field == "private_key_path" else 0o644)
...
def test_configuration_rejects_non_origin_control_urls(tmp_path: Path, control_origin: str) -> None:
    document = _valid_document(tmp_path)
...
@pytest.mark.parametrize(
    "node_id",
    ["SPK_0123456789abcdef0123456789abcdef", "spk_abc", "spk_0123456789ABCDEF0123456789ABCDEF"],
)
def test_configuration_rejects_noncanonical_node_ids(tmp_path: Path, node_id: str) -> None:
    document = _valid_document(tmp_path)
...
def test_configuration_rejects_invalid_poll_bounds(tmp_path: Path, minimum: object, maximum: object) -> None:
    document = _valid_document(tmp_path)
...
@pytest.mark.parametrize(
    "field",
    ["certificate_path", "private_key_path", "ca_path", "state_root", "installed_policy_path"],
)
...
def test_configuration_file_rejects_symlink_and_fifo_without_reading_them(tmp_path: Path) -> None:
    document = _valid_document(tmp_path)
...
def test_configuration_errors_do_not_echo_secret_values(tmp_path: Path) -> None:
    ...
    path = _write_config(tmp_path, {}, raw=("{\"enrollment_token\":\"" + sentinel).encode())
