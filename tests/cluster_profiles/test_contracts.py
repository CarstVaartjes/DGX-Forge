...
CANONICAL_SCHEMA_ROOT = REPOSITORY_ROOT / "src/cluster_profiles/schemas"
...
def test_contract_schemas_are_package_resources() -> None:
    workload_schema = resources.files("cluster_profiles").joinpath(
...
def test_repository_schema_mirrors_match_canonical_package_schemas() -> None:
    ...
    # part of the cluster_profiles runtime contract package.
...
def test_built_wheel_contains_every_canonical_schema(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(wheel) as archive:
        for schema in sorted(CANONICAL_SCHEMA_ROOT.glob("*.json")):
            assert archive.read(f"cluster_profiles/schemas/{schema.name}") == (
                schema.read_bytes()
            )
...
def test_home_workload_uses_an_immutable_image_and_declarative_adapter_commands() -> None:
    ...
    release_sha256 = "11fa4d36945ed6530daf29f8b4342feaab90ad9cd47fa505cfd9858a358ebf37"
    adapter = (
