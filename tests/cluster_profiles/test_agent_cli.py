...
from cluster_profiles import backend, cli, health, switcher
...
def deny_local_controller(monkeypatch: pytest.MonkeyPatch) -> None:
    def rejected(*args: object, **kwargs: object) -> None:
        raise AssertionError("routine vonkctl attempted a local or SSH fallback")
...
def test_documented_console_command_runs_from_outside_project_environment(
    ...
    # Break caught: `uv sync` installs dependencies but no `vonkctl` console
