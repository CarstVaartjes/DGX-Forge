...
from cluster_profiles.admission import check_admission
from cluster_profiles.catalog import Catalog, fingerprint
from cluster_profiles.contracts import WorkloadDefinition
...
def test_verified_ds4_single_profile_requires_acceptance_and_exact_profile_evidence(
    ...
    catalog.maturity_fingerprints = dict(catalog.definition_fingerprints)
    missing_exact_evidence = check_admission(
        profile, catalog, healthy_inventory, accepted={}
    )
    assert missing_exact_evidence.errors == (
        "profile has no exact accepted evidence",
    )
