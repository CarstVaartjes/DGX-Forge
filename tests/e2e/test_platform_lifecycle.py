...
def test_repository_to_running_profile_and_safe_withdrawal(tmp_path: Path) -> None:
    ...
    result = subprocess.run([ROOT / "scripts/accept-platform-lifecycle", "--host", "dynamic.example", "--display-name", "dynamic-spark", "--output", output], capture_output=True, text=True)
