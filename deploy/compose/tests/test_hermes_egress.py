...
def _tools(tmp_path: Path) -> tuple[dict[str, str], Path]:
    ...
    docker.write_text(
        "#!/bin/sh\n"
        "case \"$*\" in\n"
        "  *\"network ls\"*) printf '%s\\n' vonk-test_application vonk-test_data vonk-test_hermes-egress ;;\n"
        "  *\"inspect vonk-test_hermes-egress\"*) printf '%s\\n' 172.30.0.0/24 ;;\n"
        "  *\"inspect vonk-test_application\"*) printf '%s\\n' 172.31.0.0/24 ;;\n"
        "  *\"inspect vonk-test_data\"*) printf '%s\\n' 172.29.0.0/24 ;;\n"
        "  *) exit 41 ;;\n"
        "esac\n"
    )
    docker.chmod(0o755)
    ...
    return (
        ...
        | {
            ...
            "COMPOSE_PROJECT_NAME": "vonk-test",
...
def test_apply_is_idempotent_and_verify_never_flushes(tmp_path: Path) -> None:
    ...
    assert "-N VONK_HERMES_EGRESS" in content
    assert "-F VONK_HERMES_EGRESS" in content
    assert "-I DOCKER-USER 1 -s 172.30.0.0/24 -j VONK_HERMES_EGRESS" in content
    assert "-d 10.0.0.0/24 -j REJECT" in content
    assert "-d 169.254.0.0/16 -j REJECT" in content
    assert "-A VONK_HERMES_EGRESS -j RETURN" in content
    ...
    assert "-C VONK_HERMES_EGRESS" in content
...
def test_unresolved_or_broad_cidrs_fail_before_firewall_mutation(tmp_path: Path) -> None:
    ...
    for value in ("$UNRESOLVED", "0.0.0.0/0", "10.0.0.0/7"):
        result = subprocess.run(
            ["bash", SCRIPT, "--apply"],
            env=environment | {"VONK_MANAGEMENT_CIDRS": value},
            capture_output=True,
            text=True,
            check=False,
        )
